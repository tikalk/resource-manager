...
func (objectHandler *ObjectHandler) performObjectDelete() (err error) {
	...
	case "Namespace":
		err = objectHandler.clientset.CoreV1().Namespaces().Delete(controllers.ctx, objectHandler.fullname.Name, opts)
	case "Deployment":
		err = objectHandler.clientset.AppsV1().Deployments(objectHandler.resourceManager.Namespace).Delete(controllers.ctx, objectHandler.fullname.Name, opts)
	default:
...
func (objectHandler *ObjectHandler) Run() {
	...
	if objectHandler.resourceManager.Spec.Condition.Timeframe != "" {
		...
		objectHandler.log.Info(controllers.trace(fmt.Sprintf("object timeframe expiration <%s> timeframe <%s> age <%s> wait <%s>",
		...
		objectHandler.log.Info(controllers.trace(fmt.Sprintf("object time expiration <%s> expireAt <%s> now <%s> wait <%s>",
			objectHandler.fullname,
			expireAt.String(),
			now.String(),
			wait)))
	} else {
		objectHandler.log.Error(errors.New("expiration is not configured"), controllers.trace(fmt.Sprintf("object handler <%s> aborted", objectHandler.fullname)))
		return
	}

	if wait <= 0 {
		objectHandler.log.Info(controllers.trace(fmt.Sprintf("object already expired <%s>", objectHandler.fullname)))
	} else {
		select {
		case <-objectHandler.stopper:
			objectHandler.log.Info(controllers.trace(fmt.Sprintf("objectHandler aborted for object<%s>", objectHandler.fullname)))
			return
		case <-time.After(wait):
			objectHandler.log.Info(controllers.trace(fmt.Sprintf("object expired <%s>", objectHandler.fullname)))
			break
		}
	}

	if objectHandler.resourceManager.Spec.DryRun {
		objectHandler.log.Info(controllers.trace(fmt.Sprintf("dry-run performing object <%s> action <%s> ", objectHandler.fullname, objectHandler.resourceManager.Spec.Action)))
	} else {
		objectHandler.log.Info(controllers.trace(fmt.Sprintf("performing object <%s> action <%s>...", objectHandler.fullname, objectHandler.resourceManager.Spec.Action)))
		err := objectHandler.performObjectAction()
		if err != nil {
			objectHandler.log.Error(err, controllers.trace(fmt.Sprintf("object <%s> action <%s> failed", objectHandler.fullname, objectHandler.resourceManager.Spec.Action)))
		} else {
			objectHandler.log.Info(controllers.trace(fmt.Sprintf("object <%s> action <%s> finished", objectHandler.fullname, objectHandler.resourceManager.Spec.Action)))
		}
