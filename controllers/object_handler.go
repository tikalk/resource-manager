...
func NewObjectHandler(resourceManager *v1alpha1.ResourceManager, obj interface{}, clientset *kubernetes.Clientset, log logr.Logger) (*ObjectHandler, error) {
	...
	fullName, err := extractFullname(resourceManager.Spec.Resource, obj)
	if err != nil {
		return nil, err
	}

	creationTime, err := extractCreationTime(resourceManager.Spec.Resource, obj)
	if err != nil {
		return nil, err
	}

	terminating, err := extractTerminating(resourceManager.Spec.Resource, obj)
	if err != nil {
		return nil, err
	}
...
func (objectHandler *ObjectHandler) performObjectDelete() (err error) {
	...
	switch objectHandler.resourceManager.Spec.Resource {
	case "Namespace":
	...
	default:
		err = fmt.Errorf("objectDelete: unxpected object kind <%s>", objectHandler.resourceManager.Spec.Resource)
	}
	return err
}
...
func (objectHandler *ObjectHandler) Run() {
	...
	if objectHandler.resourceManager.Spec.Condition.ExpireAfter != "" {
		timeframe, _ := time.ParseDuration(objectHandler.resourceManager.Spec.Condition.ExpireAfter)
